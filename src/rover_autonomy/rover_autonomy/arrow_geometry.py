...
def estimate_mask_yaw_error(mask: np.ndarray, image_width: int) -> float:
    points = np.column_stack(np.nonzero(mask > 0))
    if len(points) == 0:
        return 0.0
    centroid_x = float(points[:, 1].mean())
    image_center_x = image_width / 2.0
    normalized = float((centroid_x - image_center_x) / max(image_width / 2.0, 1.0))
    normalized = max(-1.0, min(1.0, normalized))
    return normalized * (math.pi / 4.0)
